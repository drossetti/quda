...
__global__ void packFaceDWKernel(FloatN *out, float *outNorm, const FloatN *in, const float *inNorm, const int parity)
{
  ...
  template <int dim, int dagger, typename FloatN>
__global__ void packFaceDW4DKernel(FloatN *out, float *outNorm, const FloatN *in, const float *inNorm, const int parity)
{
  const int nFace = 1; // 1 face for Wilson
  const int Nint = 12; // output is spin projected
  size_t faceBytes = nFace*Ls*ghostFace[dim]*Nint*sizeof(out->x);
  ...
  // compute an index into the local volume from the index into the face
  int idx = indexFromDW4DFaceIndex<dim, 1>(face_idx, face_volume, face_num, parity);
...
template <typename FloatN>
void packFaceDW4D(FloatN *faces, float *facesNorm, const FloatN *in, const float *inNorm, 
		    const int dim, const int dagger, const int parity, 
		    const dim3 &gridDim, const dim3 &blockDim, const cudaStream_t &stream){
#ifdef GPU_DOMAIN_WALL_DIRAC  
  if (dagger) {
    switch (dim) {
    case 0: packFaceDW4DKernel<0,1><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    case 1: packFaceDW4DKernel<1,1><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    case 2: packFaceDW4DKernel<2,1><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    case 3: packFaceDW4DKernel<3,1><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    }
  } else {
    switch (dim) {
    case 0: packFaceDW4DKernel<0,0><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    case 1: packFaceDW4DKernel<1,0><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    case 2: packFaceDW4DKernel<2,0><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    case 3: packFaceDW4DKernel<3,0><<<gridDim, blockDim, 0, stream>>>(faces, facesNorm, in, inNorm, parity); break;
    }
  }
#else
  errorQuda("DW face packing kernel is not built");
...
void packFaceDW(void *ghost_buf, cudaColorSpinorField &in, const int dim, const int dagger,  const int parity, const cudaStream_t &stream) {
  ...
  {
    //printfQuda("4D PC packing start\n");
    switch(in.Precision()) {
      case QUDA_DOUBLE_PRECISION:
        packFaceDW4D((double2*)ghost_buf, ghostNorm, (double2*)in.V(), (float*)in.Norm(), 
            dim, dagger, parity, gridDim, blockDim, stream);
        break;
      case QUDA_SINGLE_PRECISION:
        packFaceDW4D((float4*)ghost_buf, ghostNorm, (float4*)in.V(), (float*)in.Norm(), 
            dim, dagger, parity, gridDim, blockDim, stream);
        break;
      case QUDA_HALF_PRECISION:
        packFaceDW4D((short4*)ghost_buf, ghostNorm, (short4*)in.V(), (float*)in.Norm(), 
            dim, dagger, parity, gridDim, blockDim, stream);
        break;
    }
  }
  ...
  {
    //printfQuda("5D PC packing start\n");
    switch(in.Precision()) {
      case QUDA_DOUBLE_PRECISION:
