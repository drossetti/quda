...
// M5inv operator
//
// In the part, we will do the following operation in parallel way.
//
// w = M5inv * v
...
#endif
{
  ...
  // s' = input vector index,
  // 'a'= kappa5

  spinorFloat inv_d_n = 1.0 / ( 1.0 + pow(2*a,Ls)*mferm);
  spinorFloat factorR;
  spinorFloat factorL;

  for(int s = 0; s < Ls; s++)
  {
    factorR = ( xs < s ? -inv_d_n*pow(2*a,Ls-s+xs)*mferm : inv_d_n*pow(2*a,xs-s))/2.0;
    factorL = ( xs > s ? -inv_d_n*pow(2*a,Ls-xs+s)*mferm : inv_d_n*pow(2*a,s-xs))/2.0;
