		      ...
		      cudaStream_t *stream_p, bool zeroCopyPack)
{
  ...
  if (zeroCopyPack) {
    ...
    in.packGhost((QudaParity)parity, dim, (QudaDirection)dir, dagger, &stream[0], my_face_d);
  } else {
    in.packGhost((QudaParity)parity, dim, (QudaDirection)dir, dagger, &stream[Nstream-1]);
  }
}
