    ...
    Float* act_path_coeff)
{
  ...
  for(int dir=XUP; dir<=TUP; ++dir){
    ...
    for(int i=0; i<V; ++i){
      ...
      llfat_scalar_mult_su3_matrix(sitelink[dir]+i, act_path_coeff[5], llink);
      dx[dir] = 1;
      int nbr_idx = neighborIndexFullLattice(i, dx[3], dx[2], dx[1], dx[0]);
  ...
  void
llfat_reference(void** fatlink, void**longlink, void** sitelink, QudaPrecision prec, void* act_path_coeff)
{
  Vs[0] = Vs_x;
  Vs[1] = Vs_y;
  Vs[2] = Vs_z;
    ...
    su3_matrix** ghost_sitelink_diag, Float* act_path_coeff)
{
  ...
  void
llfat_reference_mg(void** fatlink, void**longlink, void** sitelink, void** ghost_sitelink,
