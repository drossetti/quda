...
//    inv_param.clover_coeff = 0.0000000000000000001;
...
//  FILE *Caca = fopen("SpinorTm.In", "r+");
  ...
  printfQuda("Reading spinor\n");
  fflush(stdout);

  FILE *Caca = fopen("Result.Out", "r+");

  int		Cx,Cy,Cz,Ct,Cidx,colIdx,diracIdx;
  double	reP, imP;

  int	myRank;
  myRank	= comm_coord(3);//rank();
  ...
  do
  {
	...
	if (Cx == 0 && Cy ==0 && Cz == 0 && colIdx == 2 && diracIdx == 3)
	{
		printf("Scanning time slice %02d, Rank %d \n", Ct, myRank);
		fflush(stdout);
	}
...
int main(int argc, char **argv)
{
  ...
  char fileName[16];
  sprintf(fileName, "Spinor.Out.%d", myRank);

  FILE *Caca1 = fopen("Result.In", "r+");
  FILE *Caca2 = fopen(fileName, "r+");
  ...
  do
  {
	...
	normDiff	+= (re1P - re2P)*(re1P - re2P) + (im1P - im2P)*(im1P - im2P); 

/*	if	((normDiff/(re1P*re1P + re2P*re2P + im1P*im1P + im2P*im2P)) > 2E-6)
...
*/
  ...
  fclose(Caca2);
#endif
  printf("Done. NormDiff rank %d %le\n", myRank, normDiff);
